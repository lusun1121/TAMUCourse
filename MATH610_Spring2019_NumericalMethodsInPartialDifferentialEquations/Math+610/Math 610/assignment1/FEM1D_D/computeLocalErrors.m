...
function [local_L2_error_sqrd, local_H1_error_sqrd] = computeLocalErrors(localnodes, uh_at_localnodes, u_exact, grad_u_exact, Quad, FE_at_Quad)
...
det_B = abs(det(mat_B));    
q_points = Quad.xhat*mat_B' + repmat(localnodes(1,:),Quad.nq,1); % [nqx(p-1)] list of real quadrature points in cell not ref element
...
gradx_uh_at_qhat_points = FE_at_Quad.hat_phix*uh_at_localnodes; %[nqx1] = [nqxp] [px1]

% combine gradient components into gradient and transform onto real element
grad_uh_at_q_points = (mat_B\([gradx_uh_at_qhat_points]'))'; %[nqxdim] = ([dimxdim] [dimxnq] )'
...
for q_index = 1:Quad.nq
    ...
    diff_grad = grad_uh_at_q_points(q_index) - grad_u_exact_at_q_points(q_index); % [1x1]
