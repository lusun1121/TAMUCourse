function [mesh_size, n_dofs, L2error, H1error, Linferror] = runFEM1Dproblem(n_elements,p,quad_degree, bPlotSolutions)
...
u_exact=@(x)Q/a*(1+sinh(sqrt(a))/cosh(sqrt(a))*sinh(sqrt(a)*x/L)...
    -cosh(sqrt(a)*x/L));
...
grad_u_exact = @(x)Q/a*(sinh(sqrt(a))/cosh(sqrt(a))*sqrt(a)/L*cosh(sqrt(a)*x/L)...
    -sqrt(a)/L*sinh(sqrt(a)*x/L));
