function [mesh_size, n_dofs, L2error, H1error, Linferror] = runFEM1D2(n_elements,p,quad_degree, bPlotSolutions)

L = 1;
e=1e-4;


u_exact = @(x) (1-exp(-x/e))/(1-exp(-1/e));       
grad_u_exact = @(x) exp(-x/e)/(e*(1-exp(-1/e))); 
...
for cell = 1:T.n_elements
    ...
    cell_matrix2 = assembleLocalmasstiffness(vertices, FE_at_Quad, Quad, p);
