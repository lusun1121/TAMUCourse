...
function [T] = constructTriangulation1D2(L, num_elements,e)
%
...
T.nodes(1:num_elements/2) = 2*e*2*log(num_elements-1)*T.nodes(1:num_elements/2);
T.nodes(num_elements/2+1:T.n_nodes) = 1-2*(1-2*e*log(num_elements-1))*(1-T.nodes(num_elements/2+1:T.n_nodes));
