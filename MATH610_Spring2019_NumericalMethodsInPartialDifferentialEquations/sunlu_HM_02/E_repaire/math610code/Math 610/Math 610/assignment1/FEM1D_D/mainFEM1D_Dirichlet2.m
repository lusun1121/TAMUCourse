...
u_exact = @(x)cos(2*x)+1 ;     % if x input is [nxdim] returns [nx1]
grad_u_exact = @(x) -2*sin(2*x);  % if x input is [nxdim] returns [nxdim]

% right hand side function
f = @(x) 7*cos(2*x)+3;  % = - u''(x)   if x input is [nxdim]  returns [nx1]
...
B = spalloc(T.n_nodes, T.n_nodes, 3*T.n_nodes);
...
for cell = 1:T.n_elements
    ...
    cell_matrix1 = assembleLocalStiffness(vertices, FE_at_Quad, Quad); % assemble local stiffness terms
    cell_matrix2 = assembleLocalStiffness2(vertices,FE_at_Quad, Quad);
    ...
    A(cellIndices,cellIndices) = A(cellIndices,cellIndices) + cell_matrix1;
    B(cellIndices,cellIndices) = B(cellIndices,cellIndices) + cell_matrix2;
