...
%
%
function [uh]=mainFEM1D_Dirichlet(num_elements,problem_case,choose)
%num_elements : 10 or 20 or 40 or 80 (number of intervals)
%
%problem_case   problem
% 1             homework1
% 2             homework2
%
%choose =[quad_stiffness_n_points, quad_mass_m_points]
%different quadrature for stiffness and mass matrix
...
function_K = -1;        %K(x) = constant
function_Q = S/D;       %Q(x) = constant
...
% setup_system
%
p = 2;  % polynomial degree of lagrange finite element basis
...
quad_stiffness_n_points = choose(1);
Quad_stiffness = getQuadOnRefElement(quad_stiffness_n_points);

quad_mass_m_points = choose(2);
Quad_mass = getQuadOnRefElement(quad_mass_m_points);
...
[ FE_at_Quad_stiffness] = feEval( Quad_stiffness, p );
[ FE_at_Quad_mass] = feEval( Quad_mass, p );

quad_n_points = 4;%max(quad_stiffness_n_points,quad_mass_m_points);%4;
...
%for cell = 1:T.n_elements
    ...
    cell_matrix1 = assembleLocalStiffness(vertices, FE_at_Quad_stiffness, Quad_stiffness, p);
    cell_matrix0 = assembleLocalmass(vertices, FE_at_Quad_mass, Quad_mass, p);
    % assemble local stiffness terms
    
    cell_matrix = -function_K * cell_matrix1 ...
                  +function_Q * cell_matrix0;
...
lmbd=figure;%figure(1);
plot( T.nodes, uh(1:T.n_nodes), T.nodes, u_exact(T.nodes) ); 
% only plot the vertices with linear solution
        
xlabel('x');
ylabel('solution');
title({'1D Laplacian with Dirichlet BC',['homework ',num2str(problem_case)],...
    ['Stiffness qua=',',',num2str(choose(1)),',','mass qua=',num2str(choose(2)),',','N=',num2str(DoFHandler.n_dofs)]});
legend('uh', 'uexact');
%        saveas(lmbd, ['\\blender\homes\l\u\lusun8825\nt\AccountSettings\Desktop\sunlu\result\',...
%            'hm',num2str(problem_case),'_S',num2str(S),...
%            '_N',num2str(DoFHandler.n_dofs),'_sl.png'],'png');
...
xlabel('x');
ylabel('uh-u');
title({'Error at half nodes',['homework ',num2str(problem_case)],...
    ['Stiffness qua=',',',num2str(choose(1)),',','mass qua=',num2str(choose(2)),',','N=',num2str(DoFHandler.n_dofs)]});
    %['S=',num2str(S),',','N=',num2str(DoFHandler.n_dofs)]});
%        saveas(lmbd, ['\\blender\homes\l\u\lusun8825\nt\AccountSettings\Desktop\sunlu\result\',...
%            'hm',num2str(problem_case),'_S',num2str(S),...
%            '_N',num2str(DoFHandler.n_dofs),'_er.png'],'png');
